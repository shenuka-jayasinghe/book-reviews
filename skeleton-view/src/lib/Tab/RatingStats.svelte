<script>
	...
	let stars = [5, 4, 3, 2, 1];
	export let ratings = [5, 5, 2, 2, 3, 3, 1];
	/**
	 * @type {any[]}
	 */
	let ratingStats = [];
	...
	stars.forEach((star) => {
		...
		ratings.forEach((rating) => {
			if (rating === star) count++;
		});
        ratingNumber.push(count)
		ratingStats.push((count / ratings.length * 100).toFixed(0));
	});
</script>
	...
	<div class="flex flex-row">
		...
		<div class="basis-3/5 p-2">
			<ProgressBar
				class=""
				label="Progress Bar"
				value={ratingStats[index]}
				meter="bg-[#d6ba3e]"
				max={100}
			/>
		</div>
		<div class="basis-1/5">{`${ratingStats[index]}% (${ratingNumber[index]})`}</div>
	</div>
{/each}

<style>
	:root {
		--color-surface-900: rgb({#ded940});
	}
</style>
